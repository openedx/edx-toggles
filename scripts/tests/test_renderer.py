...
def test_transform_toggle_data_for_csv(mocked_toggle):
    ...
    assert total_num_of_loops * 2 == [
        datum["toggle_type"] for datum in output_data
    ].count("WaffleFlag")
    assert total_num_of_loops * 3 == [
        datum["toggle_type"] for datum in output_data
    ].count("WaffleSwitch")
...
def test_get_sorted_headers_from_toggles():
    ...
    sorted_header = csv_renderer.get_sorted_headers_from_toggles(
        flattened_data, ["name"]
    )
...
def test_filter_and_sort_toggles_filtering():
    ...
    data = [
        {"name": name, "toggle_type": random.choice(list(toggle_types))}
        for name in names
    ]
    ...
    tc.assertCountEqual(
        [datum for datum in data if datum["toggle_type"] == "WaffleFlag"], filtered_data
    )
    ...
    filtered_data = csv_renderer.filter_and_sort_toggles(
        data, ["WaffleFlag", "WaffleSwitch"]
    )
    tc.assertCountEqual(
        [
            datum
            for datum in data
            if datum["toggle_type"] == "WaffleFlag"
            or datum["toggle_type"] == "WaffleSwitch"
        ],
        filtered_data,
    )
...
def test_filter_and_sort_toggles_sorting():
    ...
    names = [
        "{}{}{}{}".format(*(random.choice(list("abcdefghijkl")) for num in range(4)))
        for num2 in range(30)
    ]
    data = [
        {"name": name, "toggle_type": "NAN", "NAN": random.randrange(50)}
        for name in names
    ]
