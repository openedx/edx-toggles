...
class IDA(object):
    ...
    def add_toggle_data(self, state_data_path):
        ...
        with io.open(state_data_path) as data_file:
            try:
                ...
                LOGGER.error(
                    "Loading json file at: {} failed, check toggle data in file is formatted correctly".format(
                        state_data_path
                    )
                )
                raise
    ...
    def _add_toggle_data(self, state_data):
        ...
        for toggle_type, toggles_data in state_data.items():
            for toggle_data in toggles_data:
                ...
                toggle = self._get_or_create_toggle_and_state(
                    toggle_type, toggle_name, toggle_data
                )

        LOGGER.info("Finished collecting toggle state for {}".format(self.name))
        for toggle_type in self.toggles.keys():
            LOGGER.info(
                "- Collected {}: {}".format(toggle_type, len(self.toggles[toggle_type]))
            )

    def _get_or_create_toggle_and_state(
        self, toggle_type, toggle_name, toggle_state_data=None
    ):
        """
    ...
    def add_annotations(self):
        ...
        LOGGER.info("Finished collecting annotations for {}".format(self.name))
        for toggle_type in self.toggles.keys():
            # count number of toggles that have annotations
            annotation_count = len(
                [
                    value.annotations
                    for _, value in self.toggles[toggle_type].items()
                    if value.annotations
                ]
            )

            LOGGER.info(
                "- Collected annotated {}: {}".format(toggle_type, annotation_count)
            )
    ...
    def _add_annotation_data_to_toggle_state(self, annotation_file_contents):
        ...
        def group_annotations(annotations):
            ...
            for i in set(a["report_group_id"] for a in annotations):
                group = list(filter(lambda a: a["report_group_id"] == i, annotations))
                groups.append(group)
            return groups
        ...
        for source_file, annotations in annotation_file_contents.items():
            ...
            for group in annotation_groups:
                ...
                toggle_annotation.line_numbers = [a["line_number"] for a in group]
    ...
    def _contains(self, toggle_type, toggle_name):
        ...
        try:
            present = any(
                map(lambda t: t.name == toggle_name, self.toggles[toggle_type])
            )
        except KeyError:
            return False
        return present
...
def add_toggle_annotations_to_idas(
    idas, annotation_report_files_path, idas_configuration=None
):
    """
    Given a dictionary of IDAs to consider, and the path to a directory
    ...
    annotation_files = [
        f
        for f in os.listdir(annotation_report_files_path)
    ...
    for annotation_file in annotation_files:
        ...
        if ida_name not in idas:
            idas[ida_name] = IDA(ida_name, idas_configuration.get(ida_name, None))
        LOGGER.info(
            "Collecting annotations from {} for {}".format(annotation_file, ida_name)
        )
